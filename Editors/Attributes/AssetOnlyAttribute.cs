...
namespace JxModule
{
    public class AssetOnlyAttribute : PropertyAttribute
    {
        ...
        public AssetOnlyAttribute(string message = "Asset only.")
        {
            Message = message;
        }
    }
    ...
    public class AssetOnlyAttributeDrawer : PropertyDrawer
    {
        ...
        {
            var attributeData = (AssetOnlyAttribute)attribute;
            ...
            if (property.propertyType != SerializedPropertyType.ObjectReference)
            {
                ...
                DrawHelpBox(position, "AssetOnly can only be used on ObjectReference fields.", MessageType.Warning);
                return;
            }
            ...
            if (changed)
            {
                bool isInvalid = newValue != null && !EditorUtility.IsPersistent(newValue);
